...
import { buildSeatIndex, middleColSet, positionScore, rowZone } from './layout'
...
export function computeFairness(cls: ClassEntity): FairnessReport {
  ...
  for (const asg of assignments) {
    ...
    for (const [seatId, studentId] of Object.entries(asg.map)) {
      ...
      // 前 / 中 / 后用 rowZone 同一套互斥划分：每人每周恰好计入一段
      switch (rowZone(seat.row, cls.layout)) {
        case 'front':
          frontCount.set(studentId, (frontCount.get(studentId) ?? 0) + 1)
          break
        case 'back':
          backCount.set(studentId, (backCount.get(studentId) ?? 0) + 1)
          break
        default:
          middleCount.set(studentId, (middleCount.get(studentId) ?? 0) + 1)
      }
